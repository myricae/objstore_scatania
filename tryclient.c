...
int main(int argc,char* argv[]){
    ...
    fwrite(ogg,1,9999,f);
    fclose(f);
    if(os_store("Obj1",(void*)ogg,9999)){
        printf("tryclient:store succeeded\n");
    }
    else{
        printf("tryclient:store aborted\n");
    }
    if(os_store("peccore","ad anna piacciono le peccore\n",29)){
        printf("tryclient:store succeeded\n");
    }
    else{
        printf("tryclient:store aborted\n");
    ...
    void* ret;
    if((ret=os_retrieve("peccore"))!=NULL) {
        if(ret){
        printf("tryclient: retrieve succeeded. valid pointer returned.\n");
        printf("tryclient: first 100 bytes of received file: \n");
        fwrite(ret,1,100,stdout);
        printf("\nstrncmp(ogg,ret_v,9999): %d\n",strncmp(ogg,ret,29));
        FILE *f=fopen("./out0","w");
        int scritti=fwrite(ret,1,29,f);
        printf("tryclient: fwrite: scritti: %d\n",scritti);
        if(((char*)ret)[29]=='\0') printf("l'ultimo carattere è un terminatore.\n");
        else printf("%c",((char*)ret)[29]);
        fclose(f);
        printf("\n");
        free(ret);
    }
    else{
        printf("tryclient: retrieve aborted: invalid pointer returned.\n");
    }
    }
    if(os_store("Obj3",(void*)ogg,9999)){
        printf("tryclient:store succeeded\n");
    }
    else{
        printf("tryclient:store aborted\n");
    ...
    if(os_delete("Obj1")){
        printf("tryclient:delete of %s succeeded.\n","Obj1");
    }
    else{
        printf("tryclient:delete of %s aborted.\n","Obj1");
    }
    if(ret_v){
        printf("tryclient: retrieve succeeded. valid pointer returned.\n");
        printf("tryclient: first 100 bytes of received file: \n");
        fwrite(ret_v,1,100,stdout);
        printf("\nstrncmp(ogg,ret_v,9999): %d\n",strncmp(ogg,ret_v,9999));
        FILE *f=fopen("./out","w");
        int scritti=fwrite(ret_v,1,9999,f);
        printf("tryclient: fwrite: scritti: %d\n",scritti);
        if(((char*)ret_v)[9999]=='\0') printf("l'ultimo carattere è un terminatore.\n");
        else printf("%c",((char*)ret_v)[9999]);
        fclose(f);
        printf("\n");
    }   
    else{
